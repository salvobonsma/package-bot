...
const trackUPS = async (packageNum, lang) => {
    let browser = await puppeteer.launch({});
    let page = await browser.newPage();
    var trackURL = `https://www.ups.com/track?loc=en_GB&tracknum=${packageNum}`;
    if (lang == 'PL')
        trackURL = `https://www.ups.com/track?loc=pl_PL&tracknum=${packageNum}`;
    if (lang == 'DE')
        trackURL = `https://www.ups.com/track?loc=de_DE&tracknum=${packageNum}`;
    ...
    let packages = await page.evaluate(() => {
        var pcgs = [];
        var tempPcgs = [];
        const correctNumber = $('#stApp_txtAdditionalInfoTrackingNumber').text().split('\n')[0];
        $('td').each(function () {
...
//# sourceMappingURL=trackUPS.js.map